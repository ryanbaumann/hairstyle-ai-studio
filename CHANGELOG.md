...
### Changed
- Migrated all Gemini calls to standard Models API (`models.generateContent` and `models.generateContentStream` instead of Interactions API) due to CORS preflight restrictions on browser-side calls in a pure client-side SPA.
- Set the default generation mode to 'fast' (using `gemini-3.1-flash-lite-image`).
